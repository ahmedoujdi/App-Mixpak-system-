...
import { ShieldCheck, Plus, Trash2, AlertOctagon, Camera, Image as ImageIcon } from "lucide-react";
import {
  COLORS,
  inputStyle,
  selectStyle,
...
export default function Calidad({ user }) {
  ...
  async function removeIssue(issue) {
    if (issue.photos?.length) {
      await Promise.all(issue.photos.map((p) => deleteObject(ref(storage, p.path)).catch(() => {})));
    }
  ...
  const filtered = useMemo(() => {
    return issues.filter((i) => filterStatus === "todas" || i.status === filterStatus);
  }, [issues, filterStatus]);
...
function IssueCard({ issue, onStatusChange, onOpen, onDelete }) {
  ...
  return (
    <div style={{ background: COLORS.panel, borderLeft: `5px solid ${sev.color}`, padding: "12px 14px" }}>
      ...
      <h3 onClick={onOpen} style={{ fontSize: 15, fontWeight: 600, margin: "6px 0 8px", cursor: "pointer" }}>{issue.title}</h3>
  ...
  return (
    <ModalShell onClose={onClose} title="Nueva incidencia de calidad">
      <form onSubmit={submit}>
        ...
        <Field label="Referencia (lote, orden de producción, máquina…)">
          <input value={form.reference} onChange={(e) => setForm({ ...form, reference: e.target.value })} style={inputStyle} placeholder="Ej. Lote 4521 / Línea 2" />
        </Field>
