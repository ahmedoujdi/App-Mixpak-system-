    ...
    <div>
      ...
      {pending.length === 0 ? (
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 10, marginBottom: 24 }}>
          {pending.map((p) => (
            <PersonRow key={p.id} person={p} onApprove={() => approve(p.id, true)} onRoleChange={(r) => changeRole(p.id, r)} />
          ))}
        </div>
      )}
      ...
      {approved.length === 0 ? (
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
          {approved.map((p) => (
            <PersonRow key={p.id} person={p} approved onApprove={() => approve(p.id, false)} onRoleChange={(r) => changeRole(p.id, r)} isSelf={p.id === user.uid} />
          ))}
        </div>
      )}
    </div>
...
function PersonRow({ person, approved, onApprove, onRoleChange, isSelf }) {
  return (
