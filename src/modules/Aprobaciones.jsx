...
import { COLORS, selectStyle, ghostButtonStyle, CenteredMessage, ConfirmDialog, StatCard, EmptyState } from "../shared.jsx";
...
export default function Aprobaciones({ user }) {
  ...
  async function approve(personId, aprobado) {
    ...
    <div>
      ...
      {pending.length === 0 ? (
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 10, marginBottom: 24 }}>
          {pending.map((p) => (
            <PersonRow key={p.id} person={p} onApprove={() => approve(p.id, true)} onRoleChange={(r) => changeRole(p.id, r)} onRemove={() => setConfirmRemove(p)} />
          ))}
        </div>
      )}
      ...
      {approved.length === 0 ? (
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
          {approved.map((p) => (
            <PersonRow key={p.id} person={p} approved onApprove={() => approve(p.id, false)} onRoleChange={(r) => changeRole(p.id, r)} isSelf={p.id === user.uid} onRemove={() => setConfirmRemove(p)} />
          ))}
        </div>
      )}
