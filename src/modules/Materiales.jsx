...
import { Boxes, Plus, Trash2, AlertTriangle, Minus, PackagePlus, Search } from "lucide-react";
import {
  COLORS,
  inputStyle,
  selectStyle,
...
export default function Materiales({ user }) {
  ...
  return (
    <div>
      ...
      {modalOpen && <MaterialModal item={editItem} onClose={() => setModalOpen(false)} />}
      {confirmDelete && (
        <ConfirmDialog
...
function MaterialCard({ item, onAdjust, onEdit, onDelete }) {
  ...
  return (
    <div style={{ background: COLORS.panel, borderLeft: `5px solid ${low ? COLORS.critical : COLORS.green}`, padding: "12px 14px" }}>
