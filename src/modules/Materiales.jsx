...
export default function Materiales({ user }) {
  ...
  async function adjustStock(item, delta) {
    ...
    try {
      await updateDoc(doc(db, "materials", item.id), {
        ...
        lastMovementBy: user?.email || "desconocido",
  ...
  const filtered = useMemo(() => {
    ...
    return items.filter((it) => {
      ...
      if (searchLower) {
        const text = `${it.name || ""} ${it.code || ""}`.toLowerCase();
    ...
    <div>
          ...
          onClick={() =>
            exportToCsv(
              ...
              filtered.map((it) => ({
                ...
                unidad: it.unit,
                stock: it.stock,
                stockMinimo: it.minStock,
                ubicacion: it.location || "",
                lote: it.lot || "",
                caducidad: it.expiryDate || "",
              }))
            )
          }
      ...
      {loading ? (
        ...
        <EmptyState
          ...
          message="Registra el primer repuesto o consumible de tu almacén."
          onAdd={() => {
            setEditItem(null);
            setModalOpen(true);
          }}
          addLabel="Crear primer material"
        />
      ) : (
  ...
  return (
    <div
      style={{
        ...
        borderLeft: `5px solid ${expired || low ? COLORS.critical : expiringSoon ? COLORS.safety : COLORS.green}`,
        padding: "12px 14px",
      }}
    >
      ...
      <h3 onClick={onEdit} style={{ fontSize: 15, fontWeight: 600, margin: "6px 0 2px", cursor: "pointer" }}>
        {item.name}
      </h3>
      ...
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 10 }}>
        ...
        <div style={{ display: "flex", gap: 6 }}>
          <button onClick={() => onAdjust(-1)} style={{ ...ghostButtonStyle, padding: "6px 10px" }}>
            <Minus size={14} />
          </button>
          <button onClick={() => onAdjust(1)} style={{ ...ghostButtonStyle, padding: "6px 10px" }}>
            <PackagePlus size={14} />
      ...
      {viewerIndex !== null && (
        <FotoViewer photos={item.photos} startIndex={viewerIndex} onClose={() => setViewerIndex(null)} />
      )}
    </div>
  );
}
  ...
  async function submit(e) {
    ...
    if (files.length) {
      try {
        ...
        setSaving(false);
        setUploadStatus("");
        setError("El material se guardó, pero algunas fotos no se pudieron subir.");
        return;
      }
    }
      ...
      <form onSubmit={submit}>
        ...
        <Field label="Fotos del material">
          <div style={{ display: "flex", gap: 8, flexWrap: "wrap", marginBottom: 8 }}>
            ...
            {/* Previsualizaciones de nuevas fotos */}
            {previews.map((src, i) => (
