...
export default function Mantenimiento({ user }) {
  ...
  async function removeTask(task) {
    if (task.photos?.length) {
      await Promise.all(
        task.photos.map((p) => deleteObject(ref(storage, p.path)).catch(() => {}))
      );
    }
          ...
          {columns.map((col) => (
            <div key={col.value}>
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
                {col.items.map((t) => (
                  <TaskCard key={t.id} task={t} onStatusChange={(s) => updateStatus(t.id, s)} onDelete={() => setConfirmDelete(t)} onOpen={() => setDetailTask(t)} />
                ))}
...
function TaskCard({ task, onStatusChange, onDelete, onOpen }) {
  ...
  return (
    <div style={{ background: COLORS.panel, borderLeft: `5px solid ${pr.color}`, boxShadow: isCritical ? `0 0 0 1px ${COLORS.critical}` : "none", padding: "12px 14px" }}>
    ...
    if (files.length) {
      try {
        ...
        for (const file of files) {
          const blob = await withTimeout(compressImage(file), 15000, "La foto tardó demasiado en procesarse.");
          const path = `tasks/${docRef.id}/${Date.now()}-${file.name}.jpg`;
        ...
        setSaving(false);
        setError("La orden se guardó, pero las fotos no se pudieron subir. Revisa que Storage esté activado en Firebase y que storage.rules esté publicado. Puedes añadir las fotos luego editando la orden.");
        return;
      }
    }
    setSaving(false);
    ...
    <ModalShell onClose={onClose} title="Nueva orden de trabajo">
      <form onSubmit={submit}>
        ...
        <Field label="Fotos (máquina o avería)">
          ...
          <input ref={fileInputRef} type="file" accept="image/*" capture="environment" multiple onChange={handleFiles} style={{ display: "none" }} />
        </Field>
