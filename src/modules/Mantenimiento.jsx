...
export default function Mantenimiento({ user }) {
  ...
  // Carga en vivo de tareas desde Firestore
  useEffect(() => {
    ...
    const unsub = onSnapshot(
      ...
      (err) => {
        console.error("Error al obtener tareas de mantenimiento:", err);
        setLoading(false);
      }
    );
    return () => unsub();
  ...
  const filtered = useMemo(() => {
    ...
    return tasks.filter((t) => {
      ...
      if (searchLower) {
        const text = `${t.machine || ""} ${t.title || ""} ${t.description || ""} ${t.workOrder || ""}`.toLowerCase();
        if (!text.includes(searchLower)) return false;
      }
      return true;
    });
  }, [tasks, filterType, filterPriority, search]);

  const stats = useMemo(() => {
    const s = { pendiente: 0, en_progreso: 0, espera_repuesto: 0, completada: 0, critica: 0 };
    tasks.forEach((t) => {
    ...
    <div>
      ...
      <div style={{ display: "flex", gap: 10, flexWrap: "wrap", marginBottom: 18, alignItems: "center" }}>
        <div style={{ position: "relative", flex: "1 1 200px" }}>
          ...
          <input
            ...
            placeholder="Buscar por máquina, tarea u OT"
          ...
          onClick={() =>
            exportToCsv(
              ...
              filtered.map((t) => ({
                OT: t.workOrder,
                maquina: t.machine,
                tipo: machineLabel(t.machineType),
                tarea: t.title,
      ...
      {loading ? (
        ...
        <EmptyState
          ...
          message="Registra la primera tarea de mantenimiento para empezar."
          onAdd={() => setModalOpen(true)}
          addLabel="Crear primera tarea"
        />
      ) : (
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(270px, 1fr))", gap: 16 }}>
          {columns.map((col) => (
            <div key={col.value}>
              ...
              >
                <col.icon size={15} />
                <h2 style={{ fontFamily: "'Oswald', sans-serif", fontSize: 13, textTransform: "uppercase", margin: 0 }}>
                  {col.label}
                </h2>
                <span
                  style={{
      ...
      {activeDetailTask && (
        <DetailModal task={activeDetailTask} onClose={() => setDetailTaskId(null)} />
      )}
      {confirmDelete && (
        <ConfirmDialog
          title="Eliminar tarea"
          message="Esto borrará el registro de la orden de trabajo. Esta acción no se puede deshacer."
  ...
  return (
    <div
      style={{
        ...
        boxShadow: isCritical ? `0 0 0 1px ${COLORS.critical}` : "none",
        padding: "12px 14px",
      }}
    >
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 6, fontSize: 12, color: COLORS.textMuted, marginBottom: 6 }}>
        <Package size={13} />{" "}
        <span>
          {task.machine} · {machineLabel(task.machineType)}
        </span>
      </div>
        ...
        <div style={{ display: "flex", gap: 6, marginBottom: 8, overflowX: "auto" }}>
          {task.photos.slice(0, 3).map((p, i) => (
            <img
              ...
              alt=""
              onClick={onOpen}
              style={{ width: 52, height: 52, objectFit: "cover", cursor: "pointer" }}
            />
          ))}
      ...
      <div style={{ display: "flex", flexWrap: "wrap", gap: 6, marginBottom: 8 }}>
        <span style={{ fontSize: 11, textTransform: "uppercase", fontWeight: 600, color: "#fff", background: pr.color, padding: "2px 8px" }}>
          {pr.label}
        </span>
...
function TaskModal({ user, onClose }) {
  ...
  // Liberar URLs temporales de memoria al desmontar
  useEffect(() => {
    return () => {
  ...
  async function submit(e) {
    ...
    try {
      const workOrder = `OT-${new Date().getFullYear()}-${String(Math.floor(10000 + Math.random() * 90000))}`;
      ...
      setSaving(false);
      setError("No se pudo guardar la orden. Revisa tu conexión y permisos.");
      return;
    }
    ...
    if (files.length) {
      try {
        ...
        for (let i = 0; i < files.length; i++) {
          ...
          const blob = await withTimeout(compressImage(files[i]), 15000, "La compresión tardó demasiado.");
          const result = await withTimeout(uploadToCloudinary(blob, `tasks/${docRef.id}`), 20000, "La subida de fotos tardó demasiado.");
          uploaded.push(result);
        }
        setUploadStatus("Guardando referencias...");
        await updateDoc(doc(db, "tasks", docRef.id), { photos: uploaded });
      } catch (err) {
        setSaving(false);
        setUploadStatus("");
        setError("La orden se guardó, pero no se pudieron subir las imágenes. Puedes reintentarlo editando la orden.");
        return;
      }
    }
      ...
      <form onSubmit={submit}>
        <Field label="Máquina / equipo *">
          <input
            ...
            style={inputStyle}
            placeholder="Ej. Envasadora línea 3"
          />
        </Field>
        ...
        <button
          type="submit"
          disabled={saving}
          style={{ ...primaryButtonStyle, width: "100%", justifyContent: "center", marginTop: 8 }}
        >
...
function DetailModal({ task, onClose }) {
  ...
  return (
    <ModalShell onClose={onClose} title={task.workOrder}>
      ...
      {task.description && <p style={{ fontSize: 14, marginTop: 12, whitespace: "pre-wrap" }}>{task.description}</p>}
      ...
      {viewerIndex !== null && (
        <FotoViewer photos={task.photos} startIndex={viewerIndex} onClose={() => setViewerIndex(null)} />
      )}
    </ModalShell>
  );
}
